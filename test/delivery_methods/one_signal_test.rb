...
class OneSignalTest < ActiveSupport::TestCase
  ...
  test "raises error on success with errors" do
    stub_request(:post, "https://api.onesignal.com/notifications?c=push").to_return(status: 200, body: '{"errors": ["This is an error"]}')
    assert_raises Noticed::ResponseUnsuccessful do
      @delivery_method.deliver
    end
  end
